...
SELECT e.FName, e.LName, e.MName 
FROM Employees AS e JOIN OrderDetails AS od
WHERE e.EmployeeID IN (SELECT od.OrderID WHERE TotalPrice > 1000);


SELECT  c.FName, c.LName, c.MName 
FROM Customers AS c JOIN OrderDetails AS od
WHERE c.CustomerNo IN (SELECT od.OrderID WHERE TotalPrice > 1000);